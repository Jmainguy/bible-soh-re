...
func (d *Database) GetNote(id int64) (*Note, error) {
	...
	err := d.db.QueryRow(
		...
		&note.Content, &note.CreatedAt, &note.UpdatedAt, &note.Username)
	if err != nil {
		return nil, err
	}
	return note, nil
...
func (d *Database) CreateNoteComment(noteID, userID int64, content string) (*NoteComment, error) {
	result, err := d.db.Exec(
...
func (d *Database) GetNoteComment(id int64) (*NoteComment, error) {
	comment := &NoteComment{}
	err := d.db.QueryRow(
		`SELECT c.id, c.note_id, c.user_id, c.content, c.created_at, c.updated_at, u.username
		 FROM note_comments c
		 INNER JOIN users u ON c.user_id = u.id
		 WHERE c.id = ?`,
		id,
	).Scan(&comment.ID, &comment.NoteID, &comment.UserID, &comment.Content,
...
func (d *Database) GetNoteComments(noteID int64) ([]*NoteComment, error) {
	...
	for rows.Next() {
		comment := &NoteComment{}
		if err := rows.Scan(&comment.ID, &comment.NoteID, &comment.UserID, &comment.Content,
			&comment.CreatedAt, &comment.UpdatedAt, &comment.Username); err != nil {
			return nil, err
		}
